...
class ConfigurationStorageService:
    ...
    def _encrypt_sensitive_data(data: Dict[str, Any], user_id: str) -> Dict[str, Any]:
        ...
        for field in sensitive_fields:
            if field in encrypted_data and encrypted_data[field]:
                ...
                if isinstance(encrypted_data[field], str) and encrypted_data[field].startswith("gAAAAA"):
                    continue
    ...
    def _decrypt_sensitive_data(data: Dict[str, Any], user_id: str) -> Dict[str, Any]:
        ...
        for field in sensitive_fields:
            if field in decrypted_data and decrypted_data[field]:
                ...
                if isinstance(decrypted_data[field], str) and decrypted_data[field].startswith("gAAAAA"):
                    try:
                        decrypted_value = fernet.decrypt(decrypted_data[field].encode()).decode()
    ...
    @staticmethod
    async def save_config(user_id: str, config: Union[DataSourceConfig, Dict[str, Any]]) -> DataSourceConfig:
        """
        ...
        encrypted_config = ConfigurationStorageService._encrypt_sensitive_data(config_dict, user_id)
        ...
        try:
            ...
            logger.info(f"Saved data source configuration {config_dict['id']} for user {user_id}")
    ...
    async def get_config(user_id: str, config_id: str) -> Optional[DataSourceConfig]:
        ...
        if not os.path.isfile(file_path):
            logger.warning(f"Data source configuration {config_id} not found for user {user_id}")
            return None
        ...
        try:
            ...
            config_dict = ConfigurationStorageService._decrypt_sensitive_data(encrypted_config, user_id)
            
            logger.info(f"Retrieved data source configuration {config_id} for user {user_id}")
    ...
    async def list_configs(user_id: str) -> List[DataSourceConfig]:
        ...
        for file_name in config_files:
            try:
                ...
                config = await ConfigurationStorageService.get_config(user_id, config_id)
                if config:
                    configs.append(config)
            except Exception as e:
                logger.error(f"Error loading configuration {file_name}: {str(e)}")
                
        logger.info(f"Retrieved {len(configs)} data source configurations for user {user_id}")
        return configs

    @staticmethod
    async def update_config(user_id: str, config_id: str, config: Union[DataSourceConfig, Dict[str, Any]]) -> Optional[DataSourceConfig]:
        """
        ...
        existing_config = await ConfigurationStorageService.get_config(user_id, config_id)
    ...
    async def delete_config(user_id: str, config_id: str) -> bool:
        ...
        try:
            os.remove(file_path)
            logger.info(f"Deleted data source configuration {config_id} for user {user_id}")
            return True
        except Exception as e:
