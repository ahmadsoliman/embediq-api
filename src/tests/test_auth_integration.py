...
@pytest.mark.asyncio
async def test_auth_endpoints(mock_validate_token):
    """Test the auth endpoints with a mocked token"""
    # Test token validation endpoint
    response = client.get(
        "/api/v1/auth/token", headers={"Authorization": f"Bearer {TEST_TOKEN}"}
    )
...
# This is a manual test script that can be run directly
